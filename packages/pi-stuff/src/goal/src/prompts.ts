...
function goalCompletionGuardBlock(goal: GoalPromptContext) {
	return `<goal_id>\n${escapeXmlText(goal.id)}\n</goal_id>\nThis goal_id is only the goal_complete tool stale-turn guard, not part of the objective. If and only if the goal is fully complete, pass this exact goal_id to goal_complete with the completion summary and requirement-by-requirement evidence.`;
}
...
function goalModeRules(goalLabel: string) {
	return [
		...
		`- Only call the goal_complete tool after evidence proves every requirement of ${goalLabel} is satisfied and no required work remains. Pass this exact goal_id plus one concrete proof entry per requirement, and never reuse an id from an older, stopped, replaced, or cleared turn.`,
		"- Use goal_blocked only for a true impasse with concrete evidence that user or external action is required. Report the same stable blocker through goal_blocked with a distinct attempted action and its observed failure on each consecutive Goal turn; the first two reports only record audit progress, so continue trying reasonable alternatives after them. The runtime stops the Goal only when its own three-turn audit and repeated_turns both reach 3.",
		"- Never use goal_blocked merely because work is hard, slow, uncertain, incomplete, needs ordinary clarification, or hit a recoverable failure.",
