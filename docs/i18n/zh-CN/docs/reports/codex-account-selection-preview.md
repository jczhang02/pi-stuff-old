<!-- translation-source: docs/reports/codex-account-selection-preview.md; translation-source-sha256: eea56e8433dcb3a360e169c56b5a16048a6958fa25c9c81bdece1597f491b445 -->

# Codex 账户选择 TUI 预览

捕获日期：2026-09-07。

本报告包含更新后的身份去重线框图及较早的 Host 捕获，设计见
[ADR 0035](../adr/0035-own-codex-account-selection.md) 和 Beads ps-4o1，功能尚未实现。
线框图使用示意数据；较早的 Host 样稿在内存中模拟账户和操作，OAuth 页面不打开浏览器，也不读取或保存凭据。

## 当前按身份去重的线框图

此文本线框图更新已接受的设计，并非新捕获的 Host 画面。原生登录与 work 对应同一已验证账户，因此只显示
work 和 personal。当前来源和启动默认值仍可以都是原生登录，而账户名称显示 work；分组不改变任何引用或凭据。
...
只有已验证为另一身份的原生账户才单独列出。未解析的原生来源在已验证账户项之外保持可见、可选，标记为
Identity unresolved，验证后才能激活。相同邮箱或名称不能证明匹配。本例 Statusline 显示 work，来源信息
放在账户详情中。

## 较早的按来源区分 Host 预览

以下捕获和临时交互 fixture 早于身份去重决策，保留原模拟的三个来源行，仅作历史证据，不再代表当前账户列表
契约。上述线框图不宣称取得新的 PTY 或认证验收。
...
较早的模拟通过了 13 个宽窄视图捕获点，覆盖键盘导航、模拟切换、默认值选择器、删除确认、失败展示、额度不可用、Escape 恢复和窄终端渲染。
