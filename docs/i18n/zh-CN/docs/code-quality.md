<!-- translation-source: docs/code-quality.md; translation-source-sha256: 19ea20a940791834058896d87dcc7a6d6e5d2bdd3b3718f4a7d55ba1556d05e1 -->
...
- 纯工程文档变更运行 `bun run check:docs`，检查链接、ADR 结构、截图、镜像/SHA 和文本安全，不运行完整代码检查或运行时 Tests。代码示例和删除文档不改变分类；混合变更仅根据非文档路径选择运行时 Tests。运行时 Skill、Prompt 和 Agent 定义是执行输入，不属于工程文档。代码变更开发期间运行聚焦 Tests 和 `bun run check`；`bun run verify` 将只读 Static Checks 与保守选择的离线 Tests 组合起来。
